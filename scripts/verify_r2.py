...
def main() -> None:
    ...
    if missing:
        print(f"  ⚠️  Missing R2 settings: {', '.join(missing)}")
        print("  ℹ️  R2 is not configured. The app will use local disk fallback.")
    ...
    print(f"  ✅ Signed URL TTL: {settings.r2_signed_url_ttl}s")
    ...
    try:
        ...
        print(f"  ✅ Bucket '{settings.r2_bucket_name}' exists and is accessible")
    except Exception as e:
    ...
    try:
        ...
        print(f"  ✅ Signed URL generated ({len(url)} chars)")
        print(f"     {url[:80]}...")
    except Exception as e:
    ...
    try:
        ...
        print(f"  ✅ Found {resp.get('KeyCount', 0)} objects")
    ...
    print("\n" + "=" * 60)
    print("✅ All checks passed! R2 is configured correctly.")
    print("=" * 60)
